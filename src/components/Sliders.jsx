...
const Sliders = ({ qID, ques, state, handleChange, error, overOneSubmit }) => {
  ...
  const checkValues = (oldValues, newValues, sliderId) => {
    if (sliderId === 0) {
      oldValues[1] = [
        Math.min(newValues[0], oldValues[1][0]),
        Math.max(newValues[1], oldValues[1][1]),
      ];
    }

    if (sliderId === 1) {
      oldValues[1] = [
        Math.min(newValues[0], oldValues[0][0]),
        Math.max(newValues[1], oldValues[0][1]),
      ];
    }
  ...
  const srija = (event, newValue, sliderId, activeThumb) => {
    handleChange((oldValue) => {
      ...
      //     oldVal[sliderId][1],
      //     minDistance,
      //     oldVal[sliderId][1] - minDistance
      //   );
      if (activeThumb === 0) {
        oldVal[sliderId] = [
          Math.min(newValue[0], oldVal[sliderId][1] - minDistance),
          newValue[1],
        ];
      } else {
        oldVal[sliderId] = [
          newValue[0],
          Math.max(newValue[1], oldVal[sliderId][0] + minDistance),
        ];
      }
  ...
  return (
    <label htmlFor="">
      <div className={"question-row " + (overOneSubmit ? error : "")}>
        ...
        {overOneSubmit && error && (
          <Tooltip
            title="Kindly provide a response."
            arrow
            interactive
            open={tooltipOpen}
